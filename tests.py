...
class TestB2(asynctest.TestCase):
    ...
    def upload_textfile(cls, contents="hello there", file_name='test/hello.txt'):
        ...
        contents = contents.encode(
            'utf-8')     # These fail unless encoded to UTF8
    ...
    def test_create_file_and_retrieve_by_id(self):
        ...
        bucket = self.getbucket()
        # These fail unless encoded to UTF8
        contents = 'Hello World!'.encode('utf-8')
        file = bucket.files.upload(
            contents=contents, file_name='test/hello.txt')
    ...
    def test_get_all_file_versions(self):
        ...
        print('test_get_all_file_versions: all versions: ',
              len(files['file_versions']))
    ...
    def test_delete_non_empty_bucket(self):
        ...
        assert self.b2.buckets.get(
            bucket_name=bucket.bucket_name), 'bucket should still exist'
    ...
    def test_delete_bucket(self):
        ...
        assert len(files_new) == 0, "Bucket should contain no files but contains {}".format(
            len(files_new))
        ...
        assert not self.b2.buckets.get(
            bucket_name=bucket.bucket_name), 'Deleted bucket still exists'
